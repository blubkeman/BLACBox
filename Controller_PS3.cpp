...
/* ===========================================================================
 *                     Controller_PS3 subclass functions
 * =========================================================================== */
...
Controller_PS3::Controller_PS3(Buffer * pBuffer) : Controller_Parent(pBuffer), _controller(&_Btd)
{
  ...
  _buffer->buttonLabel[4] = "Select";
  _buffer->buttonLabel[5] = "Start";
...
void Controller_PS3::begin(void)
{
  ...
  output += F(" - PS3 controller started.");
  printOutput();
  #endif
}
...
//      _onInit()
// ===================
static void Controller_PS3::_onInit(void)
{
  if (anchor != NULL) {

    #if defined(DEBUG_CONTROLLER) || defined(DEBUG_ALL)
    output = F("Controller_PS3::_onInit() - Controller initialization started.");
    printOutput();
    #endif
...
//      _onInitConnect
// ========================
void Controller_PS3::_onInitConnect(void)
{
  // ---------------------------------------------------------------
...
//      _connect
// ==================
void Controller_PS3::_connect(PS3BT * pController)
{
  // ------------------------------------------------------------------
  ...
  if ( ! _authorized(PS3_MAC_ADDRESSES, sizeof(PS3_MAC_ADDRESSES)) ) {
...
//      _disconnect()
// =======================
void Controller_PS3::_disconnect(PS3BT * pController)
{
...
//      _initCriticalFault()
// ==============================
void Controller_PS3::_initCriticalFault(void)
{
...
//      _resetCriticalFault()
// ===============================
void Controller_PS3::_resetCriticalFault(void)
{
  _faultData.badData = 0;
  _faultData.reconnect = true;
  _faultData.pluggedStateTime = 0;
...
//      _updateBuffer()
// =========================
void Controller_PS3::_updateBuffer(void)
{
  // ---------------------------------------------------------------
  // Save the previous joystick values for critical fault detection.
  // ---------------------------------------------------------------
